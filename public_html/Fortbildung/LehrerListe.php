...
$select = $db->query("SELECT `Vorname`, `Nachname`, `abgabe2016`
                      FROM `Lehrer`
                      order by Nachname
                      ");
...
foreach ($Lehrer as $Teacher) {
 echo '<p>' . $Teacher["Vorname"] . ' ' .
              $Teacher["Nachname"] . ' abgegeben: ' .
              $Teacher["abgabe2016"] . '<br>' .
              '</p>';
              $i++;
              } 
echo $i-1 ." Datensätze gefunden<br>";
